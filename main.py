...
# --- TEST 1: POSITIV - Basic Math Operations (Add, Subtract, Mult, Divide) ---
    ...
    # --- TEST 9: NEGATIV - Illegal Tilde Placement (Unary Variable Negation) ---
    code_neg_tilde_use = '''
        5 a int =:
        a~ b int =:
    '''
    run_compiler_pipeline("Negative Case 6: Illegal Tilde Usage on Variable", code_neg_tilde_use)


    # --- TEST 10: POSITIV - Basic Comparison Operations (<, >, ==, !=, <=, >=) ---
    code_basic_comparisons = '''
        ...
        a b >= res_ge bool =:
    '''
    runtime_env = run_compiler_pipeline("Positive Case 6: Basic Comparison Table (<, >, ==, !=, <=, >=)", code_basic_comparisons)
    if runtime_env and hasattr(runtime_env, 'vars'):
    ...
    # --- TEST 11: POSITIV - Complex Postfix Expression Combining Math and Comparisons ---
    code_math_comparison_mix = '''
        10 5 + 20 2 - > complex_comp bool =:
    '''
    runtime_env = run_compiler_pipeline("Positive Case 7: Combined Math and Comparison Postfix Expression", code_math_comparison_mix)
    if runtime_env and hasattr(runtime_env, 'vars'):
    ...
    # --- TEST 12: NEGATIV - Type Error (Comparing INT with BOOL) ---
    code_neg_comp_type = '''
        10 a int =:
        T b bool =:
    ...
    # --- TEST 13: NEGATIV - Assignment Type Mismatch (Saving BOOL Result into INT) ---
    ...
    # --- TEST 14: NEGATIV - Postfix Syntax Error (Chained Comparisons Without Connectors) ---
    ...
    # --- TEST 15: POSITIV - Extended Math Operations (Modulo & Power) ---
    code_extended_math = '''
        17 5 % res_mod int =:
        2 4 ^ res_pow int =:
    '''
    runtime_env = run_compiler_pipeline("Positive Case 8: Extended Math Operators (% and ^)", code_extended_math)
    if runtime_env and hasattr(runtime_env, 'vars'):
    ...
    # --- TEST 16: POSITIV - Boolean Operations and Truth Values (AND, OR, NOT) ---
    code_boolean_logic = '''
        T F & res_and bool =:
        T F | res_or bool =:
        T ! res_not bool =:
    '''
    runtime_env = run_compiler_pipeline("Positive Case 9: Boolean Logic and Log Values (&, |, !)", code_boolean_logic)
    if runtime_env and hasattr(runtime_env, 'vars'):
    ...
    # --- TEST 17: NEGATIV - Type Error (Logical Expression with Invalid Operand Types) ---
    code_neg_logic_type = '''
    ...
    # --- TEST 18: NEGATIV - Shorthand IF Branch Type Conflict ---
    ...
    # --- TEST 19: POSITIV - Correct Ternary Operand Order (a b c ?) ---
    code_shorthand_correct_order = '''
        T 100 200 ? ternary_true int =:
        F 500 600 ? ternary_false int =:
    '''
    runtime_env = run_compiler_pipeline("Positive Case 10: Correct Ternary Argument Order (a b c ?)", code_shorthand_correct_order)
    if runtime_env and hasattr(runtime_env, 'vars'):
    ...
    # --- TEST 20: POSITIV - Short Circuiting of AND & OR ---
    # Typsicherer Vergleich: 1 1 == anstelle von T T ==
    code_short_circuit_logic = '''
        F  1 1 ==  & short_and bool =:
        T  1 2 ==  | short_or bool =:
    '''
    run_compiler_pipeline("Positive Case 11: Short Circuit Logical Evaluation", code_short_circuit_logic)


    # --- TEST 21: POSITIV - Proving Short Circuiting avoids Exceptions ---
    code_prove_short_circuit = '''
        F  1 0 /  2 0 / ==  &  proof_and bool =:
        T  1 0 /  2 0 / ==  |  proof_or bool =:
    '''
    runtime_env = run_compiler_pipeline("Positive Case 12: Proving Short Circuiting (Evading DivByZero)", code_prove_short_circuit)
    if runtime_env and hasattr(runtime_env, 'vars'):
